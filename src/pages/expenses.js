...
let categoryRows = activeCategories.map( categories=>{
    ...
    return (
        ...
        <tr key={categories._id}>
            ...
            <td className={(categories.type==="expense")? "text-primary": "text-success"}>{(categories.type==="expense") ? "Expense": "Income"}</td>
            <td >P{categories.amount}</td>
            <td >{textdate}</td>
            
            <td>
              <Link className ="btn btn-primary" to={{pathname: "/editExpense",state:categories}}>Edit</Link>
            </td>
        </tr>
